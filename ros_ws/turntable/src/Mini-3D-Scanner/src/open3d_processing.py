...
class registerMulti:
    def __init__(self):
        ...
        self.sub = rospy.Subscriber("pcd_save_done", Int64, self.callback, queue_size = 100)
        self.pub = rospy.Publisher("/registered_pcd",PointCloud2,queue_size = 1)
        self.rotation_dir = Vector3()
        self.rotation_dir_init_flag = False
        self.sub_rotation_dir = rospy.Subscriber("plane_normal", Vector3, self.callback_dir, queue_size = 100)
    ...
    def registering(self):
        ...
        if self.initFlag == False:  # not the first cloud
            # read the new cloud
            print("----------------")
            rospy.logwarn("Have registered {} clouds;".format(self.registrationCount))
            print("Processing cloud {}...".format(self.cloud_index))

            print("saving results")
            
            self.cloud2 = open3d.io.read_point_cloud("/home/shabelson/Documents/studio_exps/exp1/{}.pcd".format(self.cloud_index-2))
            self.cloud1 = open3d.io.read_point_cloud("/home/shabelson/Documents/studio_exps/exp1/{}.pcd".format(self.cloud_index-1))
            ...
            if self.goodResultFlag == True:
                ...
                # downsampling
                self.cloud_base.voxel_down_sample(0.004);

                self.registrationCount += 1
                # save PCD file to local
                print ('writing')
                open3d.io.write_point_cloud("/home/shabelson/Documents/studio_exps/exp1/registerResult.pcd", self.cloud_base ,write_ascii = False)
        ...
        # the first cloud
            ...
            self.cloud_base = open3d.io.read_point_cloud("/home/shabelson/Documents/studio_exps/exp1/{}.pcd".format(self.cloud_index))
            self.cloud1 = copy.deepcopy(self.cloud_base)
            print ('writing')
            xyz_load = np.asarray(self.cloud_base.points)
            msg = PointCloud2()
            open3d.io.write_point_cloud("/home/shabelson/Documents/studio_exps/exp1/registerResult.pcd", self.cloud_base ,write_ascii = False)
    ...
    def registerLocalCloud(self, target, source):
        ...
        result_icp_p2l = open3d.registration.registration_icp(source_temp, target_temp, 0.02,
                current_transformation, open3d.registration.TransformationEstimationPointToPlane())
...
################
        ...
        ##    source_down.voxel_down_sample(radius)
        ##    target_down.voxel_down_sample(radius)
...
#### double nomral ICP
#################
        result_icp = open3d.registration.registration_icp(source_temp, target_temp, 0.01,
                p2l_init_trans_guess, open3d.registration.TransformationEstimationPointToPlane())
...
# first calculate what is the rotation direction and rotation angle
        ...
        trans_tol= 5  # transformation tolerance
        ...
        angle_with_pl_norm_tol = 0.5 # in radians (= 20 degrees)
        ...
        if ( tf[0,3] > trans_tol or tf[0,3] < -trans_tol or \
            ...
            return np.identity(4)
        elif ( abs(angle_with_pl_norm) > abs(angle_with_pl_norm_tol)):
            self.goodResultFlag = False
            print(abs(angle_with_pl_norm-0.5*math.pi))
            print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
            print("here in 3 ")
