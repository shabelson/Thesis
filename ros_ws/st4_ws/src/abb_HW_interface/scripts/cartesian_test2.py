...
try:
    ...
    def dist(p, q):
        print (p,q)
        print([(p_i - q_i) ** 2 for p_i, q_i in zip(p, q)])
...
class MoveGroupInterface(object):
    ...
    def plan_cartesian_path(self,Z,Y, scale=0.001):
        ...
        wpose = copy.deepcopy(move_group.get_current_pose().pose)
        ...
        #print (Z,Y,Posedist,len(self.wayPointsBuffer))
        #print(wpose.position.y,wpose.position.z)
        ...
        (plan, fraction) = move_group.compute_cartesian_path(
            self.wayPointsBuffer, 0.01, 0 # waypoints to follow  # eef_step
        )  # jump_threshold
...
if __name__ =="__main__":
    ...
    while not rospy.is_shutdown():
        try:
            ...
            RobotController.execute_plan(cartesian_plan)
