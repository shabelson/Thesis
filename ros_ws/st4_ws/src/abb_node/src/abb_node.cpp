...
#include "../include/abb_rapid_code_definitions.h"
...
bool RobotController::GetIsSimulation()
{
  return isSimulation;
}
void RobotController::SetIsSimulation(bool &sim)
{
  ROS_INFO("SETTIG SIMULATION %d",sim);
  isSimulation = sim;
  if (sim)
  {
    Rapid_Responce_simulator AbbSim();
...
bool RobotController::ping()
{
  ...
  int cmd_code = (int)(ROBOT_PING_CODE);
  strcpy(message, abb_comm::pingRobot(cmd_code).c_str());

  if(sendAndReceive(message, strlen(message), reply, cmd_code))
    return true;
  else
    return false;
}
    ...
    double q0, double qx, double qy, double qz)
{
  ...
  char message[MAX_BUFFER];
  char reply[MAX_BUFFER];
  int cmd_code = (int)(ROBOT_MOVE_CARTESIAN_CODE);
    ...
    double &q0, double &qx, double &qy, double &qz)
{
  char message[MAX_BUFFER];
  char reply[MAX_BUFFER];
  int cmd_code = (int)(ROBOT_GET_CARTESIAN_CODE);
  strcpy(message, abb_comm::getCartesian(cmd_code).c_str());
...
bool RobotController::setJoints(double position[])
{
  ...
  char message[MAX_BUFFER];
  char reply[MAX_BUFFER];
  int cmd_code = (int)(ROBOT_MOVE_JOINT_CODE);
    ...
    double &j4, double &j5, double &j6)
{
  char message[MAX_BUFFER];
  char reply[MAX_BUFFER];
  int cmd_code = (int)(ROBOT_GET_JOINT_CODE);
  strcpy(message, abb_comm::getJoints(cmd_code).c_str());
    ...
    double q0, double qx, double qy, double qz)
{
  ...
  if(x!=curToolP[0] || y!=curToolP[1] || y!=curToolP[2] || 
	...
	return false;
      ...
      char message[MAX_BUFFER];
      char reply[MAX_BUFFER];
      int cmd_code = ROBOT_SET_TOOL_CODE;
    ...
    double q0, double qx, double qy, double qz)
{
  ...
  if(x!=curWorkP[0] || y!=curWorkP[1] || y!=curWorkP[2] ||
     q0!=curWorkQ[0] || qx!=curWorkQ[1] || qy!=curWorkQ[2] || qz!=curWorkQ[3])
    {
      ...
      char message[MAX_BUFFER];
      char reply[MAX_BUFFER];
      ROS_INFO("TEST_WORK_OBJECT");
      int cmd_code = (int)ROBOT_SET_WOBJ_CODE;
...
bool RobotController::setSpeed(double tcp, double ori)
{
  ...
  if(tcp!=curSpd[0] || ori!=curSpd[1])
	...
	return false;
      ...
      char message[MAX_BUFFER];
      char reply[MAX_BUFFER];
      int  cmd_code = ROBOT_SET_SPEED_CODE;
      strcpy(message, abb_comm::setSpeed(tcp, ori,cmd_code).c_str());
      if(sendAndReceive(message, strlen(message), reply, cmd_code))
	{
 ...
 // Only take action if the required values are different than the actual ones
  if(z!=curZone)
	...
	return false;
      ...
      char message[MAX_BUFFER];
      char reply[MAX_BUFFER];
      int cmd_code = ROBOT_SET_ZONE_CODE;
...
{
  //NOTE - NOT SURE FOR WHAT IS THIS OPTIONS...
  char message[MAX_BUFFER];
  char reply[MAX_BUFFER];
  int cmd_code = (int)(ID_CODE_MAX*(double)rand()/(double)(RAND_MAX));
  strcpy(message, abb_comm::specialCommand(command, param1, param2, param3, param4, param5, cmd_code).c_str());
...
bool RobotController::setVacuum(int v)
{
  ...
  if(v!=curVacuum)
      ...
      if((v != VACUUM_OPEN) && (v != VACUUM_CLOSE))
	{
      ...
      strcpy(message, abb_comm::setVacuum(v, cmd_code).c_str());
...
{
  //TODO - RAPID SERVER DOESNT SUPPOER SUPPORT SETDIO
  char message[MAX_BUFFER];
  char reply[MAX_BUFFER];
  int cmd_code = (int)(ID_CODE_MAX*(double)rand()/(double)(RAND_MAX));
  strcpy(message, abb_comm::setDIO(dio_num, dio_state, cmd_code).c_str());
  if(sendAndReceive(message, strlen(message), reply, cmd_code))
    { return true; }
  else 
    { return false; }
...
{
  //ROS_INFO("%d _ SIMULATION PARAM",GetIsSimulation());
  if (GetIsSimulation())
  {
    ROS_INFO("THIS IS A SIMULATED CONNECTION - communication will only echo back");
    ...
    char* reply, int idCode)
{
      ...
      if ((t=recv(robotMotionSocket, reply, MAX_BUFFER-1, 0)) > 0)
      {
        ...
        if(idCode!=-1)
        {
          ...
          if ((ok == SERVER_OK) && (rcvIdCode == idCode))
          {
            ...
            //ROS_INFO("MSG SENT send_code: %s reply_code: %s:",idCode,rcvIdCode);
            //ROS_INFO("TEST4");
        ...
        else
        {
          if (ok == SERVER_OK)
          {
            //ROS_INFO("MSG SENT send_code: %d reply_code: %s:",idCode,rcvIdCode);
            pthread_mutex_unlock(&sendRecvMutex);
            return true;
          }
  ...
  else if (GetIsSimulation())
  {
    //ROS_INFO("SIM MSG SENT");
    string DataRecived = AbbSim.ServerIncomeMsg(message);
    long t = DataRecived.length();
    // declaring character array
    char char_array[t + 1];
    strcpy(char_array, DataRecived.c_str());
    char_array[t] = '\0';
    int ok, rcvIdCode;
    sscanf(char_array,"%d %d", &rcvIdCode, &ok);
    //ROS_INFO("msg = %s, reply = %s, idCode = %d, ok = %d, rcvCode = %d", message, char_array, idCode, ok, rcvIdCode);
    reply = char_array;
    //ROS_INFO(reply);
    return true;
    //use a RAPID MIMIC RESPONCE to simualate the behavior of the robot responce -
    //That way the message debugging will be as expected.
    //SimResponce(message);
  }
}
...
void RobotController::logCallback(const ros::TimerEvent&)
{
  ...
  {
    t = recv(robotLoggerSocket, buffer, MAX_BUFFER-1, 0);
  }
  if ((t) > 0)
  {
    ...
    while((partialBuffer = strchr(partialBuffer,'#'))!=NULL)		
    {
      ...
      switch(code)
      {
        ...
        char date[MAX_BUFFER];
            ...
            int nParams = sscanf(buffer,"# %*d %s %s %*f %lf %lf %lf %lf %lf %lf %lf",
         code,
         date,
				 time,
         temp,
...
int main(int argc, char** argv)
{
  ...
  boolalpha;
  const std::string rosNodeName =  "abb_robot_controller";
  ros::init(argc, argv,rosNodeName);
  ros::NodeHandle node;
  bool sim;
  node.getParam("/abb_node/sim",sim);
  ROS_INFO("ROS SIMAULATION := %d",sim);
  RobotController ABBrobot(&node);
  ABBrobot.SetIsSimulation(sim);
