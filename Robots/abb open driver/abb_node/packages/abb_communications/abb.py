...
class Robot:
    ...
    def load_json_tool(self, file_obj):
        if file_obj.__class__.__name__ == 'str':
            file_obj = open("tool.object.json", 'rb');
    ...
    def set_external_axis(self, axis_unscaled=[-55,0,0,0,0,0,0]):
    ...
    def format_pose(self, pose):
        ...
        for cartesian in pose[0]:
            print (type(self.scale_linear),type(cartesian))
...
if __name__ == '__main__':
    ...
    abb = Robot()
    abb.get_robotinfo()
    curPose = abb.get_cartesian()
    
    arcStart = copy.deepcopy(curPose)
    arcEnd = copy.deepcopy(curPose)
    print (type(arcStart[0][2]))
    arcStart[0][2] += 50
    
    arcEnd[0][2] += 50
    arcEnd[0][1] += 50
    log.info("ATTEMPT TO CIRULAR MOVEMET")
    arcStr1 = [str(i) for l in arcStart for  i in l]
    arcStr2 = [str(i) for l in arcEnd for  i in l]
    print (arcStr1)
    print (arcStr2)
    #abb.move_circular(arcStart,arcEnd)
    abb.set_joints([0,0,0,0,0,0])
    abb.close()
